...
Im Rahmen des *TI-Messenger-Dienstes* ist es notwendig, dass die Relying Party (*Registrierungs-Dienst*) sich beim zentralen *IDP-Dienst* registriert, bevor die *Relying Party* am vom *IDP-Dienst* angebotenen Authentifizierungsprozess teilnehmen kann. Die Registrierung erfolgt hierbei als organisatorischer Prozess. Erst nach erfolgter Registrierung, bei der die Adresse der *Relying Party* (`redirect_uri`), sein öffentlicher Schlüssel und die von ihm erwarteten Attribute, in Form von Claims, angegeben wurden, kann der IDP-Dienst ein `ID_TOKEN` ausstellen. 
