...
Die Hersteller von TI-Messenger Diensten müssen breits erwähnt die Testtreiberschnittstelle bereitstellen. Bei Problemen im Rahmen der Zulassung müssen die Anbieter bei der Analyse unterstützen. In der folgenden Abbildung ist eine Systemumgebung für den Zulassungstest TI-Messenger Pro dargestellt.
...
In der folgenden Abbildung ist eine Systemumgebung für Herstellertests TI-Messenger Pro dargestellt. Bei Problemen im Rahmen der IOP Tests MÜSSEN die Anbieter des Fachdienstes und des FdVs bei der Analyse unterstützen.
...
Die Referenz-Instanz und die Test-Instanz wird auch im Anschluss der Zulassung weiter für IOP und Continuous Testing genutzt.