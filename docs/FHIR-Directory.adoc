...
=== Endpoint: /search
==== Authentication
TIP: The `/search` endpoint requires a valid JSON Web Token (`tim-authenticate-token`) in the Authorization header. 

In order to get the JSON Web Token the following steps must be performed.
...
BODY
    ...
    "user":"@user:matrix.dev.service-ti.de",
    "password":"password"}

RESPONSE
STATUS 200
HEADER
  {"date"=>"Thu, 27 Oct 2022 09:16:30 GMT", 
  "server"=>"Synapse/1.53.0", 
  "content-type"=>"application/json", 
