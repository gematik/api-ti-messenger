...
The */owner* endpoint requires a valid JSON Web Token (`owner-authenticate-token`) in the Authorization header. 

To get the token you need a 3rd Party Token from a permitted Registrierungs-Dienst (`registration-service-token`). With this token you can get the `owner-authenticate-token` from the */owner-authenticate* endpoint from the VZD-FHIR-Directory Auth Service. There are two steps necessary:
