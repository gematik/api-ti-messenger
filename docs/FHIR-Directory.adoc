...
The */owner* endpoint requires a valid JSON Web Token (`owner-authenticate-token`) in the Authorization header. In order to get the JSON Web Token the following steps must be performed.

.*1. Login to the TI-Messenger Registrierungs-Dienst as Org-Admin*
...
.*2. Get the Registrierungs-Dienst token*
