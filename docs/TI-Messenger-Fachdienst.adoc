...
As a further function, the Registrierungs-Dienst of a TI Messenger backend offers the provision of a federation list for the Messenger Proxies of its Messenger-Services.
...
The Registrierungs-Dienst must offer the Messenger-Proxies of the TI Messenger backend a function via the `I_internVerification` interface, with which it is possible to check for MXID entries in the VZD-FHIR directory.
