...
=== Messenger-Proxy
Der Messenger-Proxy als Prüfinstanz aller eingehenden `Invite Events` ist für die Regelung der gemäß Matrix Client-Server-API und Matrix-Server-Server-API geltenden Aufrufe zuständig. Bei erfolgt wird das `Invite Event` an den zuständigen Matrix-Homeserver weitergeleitet. Im folgenden werden die Funktionsmerkmale die der Messenger-Proxy umsetzen muss beschrieben. 
